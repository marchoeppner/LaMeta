...
checkm<-checkm[checkm$V2!="k__Bacteria (UID203)" & !grepl("^F",rownames(checkm)) & checkm$V12>=80,]

checkm.good=checkm[checkm$V12>=80 & checkm$V13<10 & checkm$V14<10,]

#gff<-read.gff(paste0(candidate,".coding.gff"))
#gff.cds<-gff[gff$type=="CDS",]
#gff.ranges<-GRanges(seqnames=Rle(gff.cds$seqid), ranges=IRanges(gff.cds$start,gff.cds$end), strand="+")
...
sl<-slice(cranges.cov,lower=nrow(checkm.good)*.5)
...
#noncoveredref<-subsetByOverlaps(allranges[[ref]],covered,invert=T)
